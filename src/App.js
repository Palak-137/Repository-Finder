  ...
  const listItems = repos.map((value,index) =>
      <Draggable  key={value.id} draggableId={JSON.stringify(value.id)} index={index}>
        {(provided)=>(
            ...
            <div className="profile-image"><a href={value.owner.html_url}><img src={value.owner.avatar_url} alt="profile-sample9" /></a>
              <div className="icons" style={{color:"white", fontSize: "2rem"}}>
                <a href={value.html_url}  style={{color:"white", fontSize: "2rem"}}><i className="fa fa-github" style={{fontSize: "2rem"}}></i></a>
              </div>
            </div>
            <figcaption>
              <h3>{value.name}</h3>
            ...
            <div className="container boxes" style={{width:"80%", alignItems:"center"}}>
              ...
              <DragDropContext onDragEnd={handleDrag}>
                <Droppable droppableId="characters">
                  {(provided)=>(
                      <div id="characters" className="row m-5 mt-5"  style={{alignItems:"center"}} {...provided.droppableProps} ref={provided.innerRef}>
